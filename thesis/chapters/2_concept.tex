...
\label{sec:svg_graphical_elements}
\section{Az SVG grafikus elemei}

A Scalable Vector Graphics (SVG) egy XML-alapú nyelv, mely kétdimenziós vektorgrafikák leírására használatos. Hierarchikusan felépített, minden grafikus objektum egy közös gyökérelembe, az \lstinline[language=XML]|<svg>| kezdő és záró címke közé ágyazódik. Ezt a szerkezetet szemlélteti az alábbi kódrészlet:
...
\paragraph{A \texttt{viewBox} attribútum}
A teljes skálázhatóság és a reszponzív viselkedés kulcsa az \lstinline[language=XML]|<svg>| elem opcionális \lstinline[language=XML]|viewBox| attribútuma. Ez a nézetablak meghatároz egy új, független koordináta-rendszert a tartalom számára, amely automatikusan skálázódik a megadott \texttt{width} és \texttt{height} méretekhez. Ennek hiányában a koordináták közvetlenül a képernyő pixeleiben értendők. A \texttt{viewBox} értéke négy szám: \texttt{min-x}, \texttt{min-y}, \texttt{width} és \texttt{height}. Az alábbi példában a \texttt{viewBox} egy 0-tól 100-ig terjedő koordináta-rendszert határoz meg, és az SVG tartalma teljesen kitölti a 400$\times$400 pixeles tárolót.

\begin{figure}[H]
    \centering
    \begin{adjustbox}{valign=m, max width=0.7\textwidth}
        \begin{lstlisting}[language=XML]
        <svg xmlns="http://www.w3.org/2000/svg"
    ...
    \begin{adjustbox}{valign=m, max width=0.2\textwidth}
        \includesvg{images/concept/viewbox.svg}
    \end{adjustbox}
    \caption{Példa: A viewBox attribútum hatása a koordinátarendszer skálázására}
    \label{fig:example_viewbox}
\end{figure}

Ebben a fejezetben bemutatom az SVG gyakran használt elemeit, amiket a webalkalmazásban is implementálni fogok. Kezdve a megjelenési attribútumokkal(\ref{sub:vis_attrs}), melyek a későbbiekben bemutatott elemek vizuális megjelenését befolyásolják. Folytatva az alapvető alakzatokkal(\ref{sub:basic_shapes}), melyeket először az attribútumaik listázásával, majd egy kódpéldával és ábrával demonstrálok. Ez után az SVG specifikáció különböző görbéi, az útvonalak(\ref{sub:paths}) és további grafikus elemei(\ref{sub:other_elements}) kerülnek bemutatásra, hasonló formátumban. Végül ezen elemek csoportosítását és újrafelhasználását(\ref{sub:group_defs}) mutatom be két példán keresztül.

\label{sub:vis_attrs}
\subsection{Megjelenési attribútumok}

Az SVG elemek geometriai meghatározása mellett a kinézetüket a megjelenési attribútumok\cite{mdn-svg-attributes} határozzák meg. Ezek az attribútumok CSS tulajdonságként is értelmezhetők, így megadhatók közvetlenül az elemen attribútumként, vagy CSS stíluslapokon keresztül is.
...
\textbf{stroke-width:} A körvonal vastagságát definiálja. Értéke egy pozitív szám, mely az aktuális koordináta-rendszer egységében értendő, alapértéke az 1.
...
\begin{itemize}
    ...
    \item \texttt{round:} A vonal végét egy félkör zárja le, melynek sugara a vonalvastagság fele.
    \item \texttt{square:} A vonal vége egy négyzetes lezárást kap, mely a vonalvastagság felével túlnyúlik a végponton.
\end{itemize}

\begin{figure}[H]
    \centering
    \includesvg[width=0.2\textwidth]{images/concept/stroke-linecap.svg}
    \caption{Példa: A stroke-linecap attribútum három lehetséges értéke. }
    \label{fig:example_stroke-linecap}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includesvg[width=0.2\textwidth]{images/concept/stroke-linejoin.svg}
    \caption{Példa: A stroke-linejoin attribútum három lehetséges értéke. }
    \label{fig:example_stroke-linejoin}
\end{figure}

\textbf{stroke-dasharray}: Szaggatott vonalak létrehozására szolgál. Értéke egy vesszővel vagy szóközzel elválasztott számsorozat, mely felváltva határozza meg a vonalszakaszok és a szünetek hosszát.
...
\textbf{fill-rule:} Önmagát metsző vagy lyukas útvonalak esetén határozza meg, hogy mely területek számítanak belülnek és melyek kívülnek.
...
\begin{figure}[H]
    \centering
    \includesvg[width=0.2\textwidth]{images/concept/fill-rule.svg}
    \caption{Példa: A fill-rule attribútum két lehetséges értéke. }
    \label{fig:example_fill-rule}
\end{figure}
...
\subsubsection{Vágás}
A \texttt{clip-path} egy specifikus SVG mechanizmus, mely a látható terület korlátozására szolgál. Egy SVG alakzat, például egy kör vagy vonal segítségével vágja le a megjelenítendő elemet. A definiált vágási területen kívül eső részek a megjelenítéskor nem láthatók.
...
\label{sub:basic_shapes}
\subsection{Alapvető alakzatok}

Az SVG szabvány számos előre definiált alakzatot\cite{mdn-svg-elements} támogat, melyek segítségével egyszerűen hozhatunk létre grafikai elemeket. 

\subsubsection{Vonalak}

A vonalakat a \lstinline[language=XML]|<line>| elem segítségével ábrázoljuk. A vonal meghatározásához négy alapvető attribútumot szükséges megadni, melyek a kezdő- és végpont koordinátái.
...
\begin{figure}[H]
    ...
    \end{adjustbox}
    \caption{Példa: Téglalap elem, opcionálisan lekerekített sarkakkal, kód és vizuális megjelenés.}
...
A \lstinline[language=XML]|<polygon>| elem egy tetszőleges számú éllel rendelkező, zárt alakzatot definiál. A poligon leírásához a \texttt{points} attribútum szükséges. A bejárás sorrendjében, szóközzel elválasztva szükséges leírni az x,y koordináta páronkat. A leírás végeztével automatikusan összeköti a kezdő- és a végpontot, nem szükséges ismételten megadni a kiinduló pontot.
...
Az alábbi útvonal egy sokszöget rajzol, mely a (20,20) pontban kezdődik, és a \texttt{Z} paranccsal zárul vissza a kezdőponthoz.
...
A Bézier-görbék\cite{mdn-bezier} matematikai görbék, melyek kontrollpontok segítségével definiálják a görbe alakját. Az SVG két típusú Bézier-görbét támogat.
...
Az \texttt{A, a} (Arc) parancs elliptikus ívek rajzolására szolgál. Használata előtt szükséges egy adott kezdőpont, vagy annak meghatározása az \texttt{M, m} paranccsal. Ez az egyik legbonyolultabb útvonalparancs, mivel számos paramétert igényel:
...
\item \texttt{large-arc-flag}: Meghatározza, hogy a rövidebb (0, alapérték) vagy hosszabb (1) ívet rajzolja-e
\item \texttt{sweep-flag}: Meghatározza, hogy pozitív (1, alapérték) vagy negatív (0) irányban rajzolódik-e az ív
...
\begin{figure}[H]
    ...
    <path d="M 25,50
        A !\textcolor{red}{29.5,50}! 0 0 0 75,50"
    ...
    \hfill
    \begin{adjustbox}{valign=m, max width=0.2\textwidth}
        \includesvg{images/concept/path-arc-radi.svg}
    \end{adjustbox}
    \caption{Példa: Az ív rx és ry paramétereinek hatása. A kék ív a képzeletbeli ellipszis egy szakasza. A pirossal kiemelt vonalak a sugarakat jelölik.}
    \label{fig:example_arc_radi}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{adjustbox}{valign=m, max width=0.8\textwidth}
        \begin{lstlisting}[language=XML, escapeinside={!}{!}]
    <path d="M 25,50
        A 29.5,50 !\textcolor{red}{25}! 0 0 75,50"
        fill="none" stroke="blue" stroke-width="3">
    </path>
        \end{lstlisting}
    \end{adjustbox}
    \hfill
    \begin{adjustbox}{valign=m, max width=0.2\textwidth}
        \includesvg{images/concept/path-arc-rot.svg}
    \end{adjustbox}
    \caption{Példa: Az ív x-axis-rotation paraméterének használata, kód és vizuális megjelenés}
    \label{fig:example_arc_rot}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{adjustbox}{valign=m, max width=0.8\textwidth}
        \begin{lstlisting}[language=XML, escapeinside={!}{!}]
    <path d="M 25,50
        A 29.5,50 25 !\textcolor{red}{1}! 0 75,50"
        fill="none" stroke="blue" stroke-width="3">
    </path>
        \end{lstlisting}
    \end{adjustbox}
    \hfill
    \begin{adjustbox}{valign=m, max width=0.2\textwidth}
        \includesvg{images/concept/path-arc-large.svg}
    \end{adjustbox}
    \caption{Példa: Az ív large-arc-flag paraméterének használata, a szaggatott vonal jelzi az ívet a másik lehetséges érték esetén}
    \label{fig:example_arc_large}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{adjustbox}{valign=m, max width=0.8\textwidth}
        \begin{lstlisting}[language=XML, escapeinside={!}{!}]
    <path d="M 25,50
        A 29.5,50 25 0 !\textcolor{red}{1}! 75,50"
        fill="none" stroke="blue" stroke-width="3">
    </path>
        \end{lstlisting}
    \end{adjustbox}
    \hfill
    \begin{adjustbox}{valign=m, max width=0.2\textwidth}
        \includesvg{images/concept/path-arc-sweep.svg}
    \end{adjustbox}
    \caption{Példa: Az ív sweep-flag paraméterének használata, a szaggatott vonal jelzi az ívet a másik lehetséges érték esetén}
    \label{fig:example_arc_sweep}
\end{figure}
...
Viszont egyéb CSS tulajdonság is használható, például \texttt{font-family, font-size}, melyek a betűtípust és a szöveg méretét határozzák meg. Azonban a böngészőktől és rendszerektől függő betűtípus-besorolás kompatibilitási problémákat okozhat. Egy, a szerkesztőben megadott \texttt{font-family} érték más környezetben eltérő betűtípust aktiválhat, vagy akár a tartalmat megváltoztathatja. Ennélfogva az SVG szerkesztők a szövegeket általában \lstinline[language=XML]|<path>| elemekké alakítják.
...
\subsubsection{Definiálás}
A \lstinline[language=XML]|<defs>| elem\cite{mdn-defs} későbbi felhasználásra szánt grafikus objektumok tárolására szolgál. A tárolt objektumok nem jelennek meg közvetlenül, ehhez hivatkozni kell rájuk. Ennek egyik gyakori módja például a \lstinline[language=XML]|<use>| elem használata.

A korábbi példákon látható rács is ezzel a módszerrel készült, így könnyen és egyértelműen elkülöníthető az ábra lényegi része, és egy újrahasznált elem. Az alábbi példában egy kör többszöri felhasználását mutatom be, a \lstinline[language=XML]|<defs>| között egyszer meghatározva, majd a \lstinline[language=XML]|<use>| elemmel háromszor megjelenítve, különböző helyeken.
...
\begin{figure}[H] \centering
    ...
    \hfill
    \begin{adjustbox}{valign=m, max width=0.2\textwidth}
        \includesvg{images/concept/defs.svg}
    \end{adjustbox}
    \caption{Példa: Egy elem definiálása majd többszöri használata, kód és megjelenés}
    \label{fig:example_defs}
\end{figure}

\label{sec:svg_editors}
...
\begin{figure}[H]
\centering
\includegraphics[height=0.4\textwidth]{images/concept/svg-edit.JPG}
\caption{Az SVG-Edit kezelőfelülete\cite{svg-edit}}
\label{fig:svg-edit}
\end{figure}

Az SVG-Edit (\ref{fig:svg-edit}. ábra) egy nyílt forráskódú, ingyenes webalapú szerkesztő. Használatához nincs szükség regisztrációra vagy előfizetésre. A szerkesztő fejlécében lévő menüsor a fájl kezelésére, nézet beállítására és transzformációk beállítására szolgál.
...
\begin{figure}[H]
\centering
\includegraphics[height=0.4\textwidth]{images/concept/inkscape.JPG}
\caption{Az Inkscape kezelőfelülete\cite{inkscape}}
\label{fig:inkscape}
\end{figure}

Az Inkscape (\ref{fig:inkscape}. ábra) egy ingyenes, nyílt forráskódú asztali vektorgrafikus szerkesztő. Felülete rendkívül összetett, bal oldalon a részletes eszköztár, felül az eszközvezérlő sáv és a menüsor, jobb oldalon dokkolható párbeszédablakok, alul pedig a színpaletta és az állapotsor található.
...
\item[Szövegek szerkesztése:] Van-e lehetőség több soros, bekezdéses szöveg beszúrására, vagy csak egy sorba kerül. A szerkesztőnek biztosítania kell a betűtípus, méret, stílus és igazítás beállításait. A szöveg szerkesztése közvetlenül a vásznon \textit{"in-place"} történik-e. Alternatívan egy felugró ablakban vagy külön beviteli mezőben.

\item[Színek és stílusok megadása:] Az SVG stílusozás és CSS kezelése.
\begin{itemize}
    \item A kitöltés és körvonal támogatja-e az RGB, HEX, HSL\cite{color-codes} színmegadást, valamint az átlátszóságot.
...
\item[Objektumok automatikus igazítása:] Rendelkezik-e a szerkesztő "mágneses" funkcióval, mely az objektumokat rácspontokhoz, más objektumok éleihez vagy középpontjához igazítja mozgatás közben.
